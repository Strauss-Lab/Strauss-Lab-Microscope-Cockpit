...
MASTER_IP = '10.122.218.70' # Host Computer IP; this must match the UDP Destination in LabVIEW
UPDATE_RATE = 0.001
...
class NIcRIO(executorDevices.ExecutorDevice):
    ...
    def __init__(self, name, config):
        ...
        self.nrDigitalLines = int(config.get('nrdigitallines',0))
        self.nrAnalogLines = int(config.get('nranaloglines',0))
        self.config = config
...
class Connection:
    ...
    def __init__(self, parent, ipAddress, port, localIp):
        ...
        self.errorCodes = {'0': None,
                           ...
                           '3': 'Send error'}
        self.status = None
        self.lock = threading.Lock() # this lock ensures that only one instance of runCommand() is running
    ...
    def runCommand(self, command, args=(), msgLength=20):
        ...
        """
        # Retrieve the lock; this prevents interleaved command upon device initialization
        with self.lock:
            ...
            messageCluster = {'Command': str(command),  # NOTE: Changed here. The command type should have been a number
            ...
            try:
                ...
                try:
                    ...
                    if error['status'] != 'OK':
                    # if error['status']:
                        print(f'While sending message: {messageCluster}, There has been an FPGA error: {error}')
                except json.JSONDecodeError as e:
                    print(f'While sending message: {messageCluster}, Received malformed JSON Data: {e}\t datagram: {datagram}')
                except socket.timeout as e:
                    print(f'While sending message: {messageCluster}, Connection timed out while waiting for confirmation.{e}')
                except Exception as e:
                    print(f'While sending message: {messageCluster}, We received a TCP error when confirming command: {e}')
    ...
    def PrepareActions(self, actions, numReps):
        ...
        if isinstance(self.parent, NIcRIO):
            actions = self.parent._adaptActions(actions)
...
class FPGAStatus(threading.Thread):
    ...
    def run(self):
        ...
        while self.shouldRun:
            ...
            if current_time - last_published_time >= 1.0:
                # Force a status publish
                if newFPGAStatus['Event'] == 'done':
