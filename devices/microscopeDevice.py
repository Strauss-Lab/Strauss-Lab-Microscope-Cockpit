...
class MicroscopeLaser(MicroscopeBase):
    ...
    def _setEnabled(self, on):
        if on:
            self._proxy.enable(wavelength=self.config.get('wavelength', None))
        else:
            self._proxy.disable(wavelength=self.config.get('wavelength', None))
    ...
    def _setPower(self, power: float) -> None:       
        # self._proxy.wl = self.config.get('wavelength', None)
