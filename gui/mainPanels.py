...
class fpgaAnaloguePanel(wx.Panel):
    def __init__(self, parent, line):
        super().__init__(parent, style=wx.BORDER_RAISED)
        self.line = line
        self.Sizer = wx.BoxSizer(wx.VERTICAL)
        self.valCtrl = safeControls.SafeSpinCtrlInteger(self, inc=1, minValue=line.minValue, maxValue=line.maxValue)
        self.valCtrl.Bind(safeControls.EVT_SAFE_CONTROL_COMMIT,
                          lambda evt: self.line.setValue(evt.Value))
        line.addWatch('valueSetPoint', self.valCtrl.SetValue)
        self.valCtrl.SetValue(line.valueSetPoint)
        self.Sizer.Add(wx.StaticText(self, label=line.name + ': Analogue ' + str(line.trigLine)+' Control'),
                       flag=wx.ALIGN_CENTER_HORIZONTAL)
        self.Sizer.Add(self.valCtrl,flag=wx.EXPAND)
...
class fpgaAnalogueControlsPanel(wx.Panel):
    """Create a controls panel for each FPGA analogue line."""
    def __init__(self, parent):
        super().__init__(parent)
        self.Sizer = wx.BoxSizer(wx.VERTICAL)
