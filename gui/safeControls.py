...
## Strauss Lab Customization
class SafeSpinCtrlInteger(SafeControl, wx.Panel):
    """A cross-platform spin control for integer values.
    ...
    def __init__(self, *args, minValue=0, maxValue=10, inc=1, value=0, **kwargs):
        """Initialize a SafeSpinCtrlInteger"""
        ...
        self._places = 0  # Not needed for integers

        te = self.te = wx.TextCtrl(self)
        if 'size' in kwargs:
            self.te.SetInitialSize(kwargs.get('size'))
        else:
            longest = max(len(str(minValue)), len(str(maxValue)))
            self.te.SetInitialSize(te.GetSizeFromTextSize(te.GetTextExtent('0' * longest)))
        self.te.SetValidator(IntValidator())
        sb = wx.SpinButton(self)
        self.Sizer = wx.BoxSizer(wx.HORIZONTAL)
        self.Sizer.Add(self.te, proportion=1)
        self.Sizer.Add(sb, proportion=0)
        self.Fit()
        sb.SetMaxSize((-1, te.GetSize()[-1]))
        self.SetValue(value)
        self.te.SetValue(str(value))
        self.SetBackgroundColour(self.te.GetBackgroundColour())
        sb.SetRange(0, (self._max - self._min) // self._inc)
        sb.SetValue((value - self._min) // self._inc)
        self.Bind(wx.EVT_SPIN, self.OnSpin, sb)
        self.Bind(wx.EVT_CHAR_HOOK, self.OnChar, self.te)
        self.Bind(wx.EVT_TEXT, self.OnText, self.te)
        self.Bind(wx.EVT_MOUSEWHEEL, self.OnMouseWheel, self.te)
        self.Bind(wx.EVT_TIMER, self.CheckFocus)
        self.Bind(wx.EVT_SET_FOCUS, self.OnFocus, self.te)
        self.Bind(wx.EVT_CHILD_FOCUS, self.OnFocus, self.te)
    ...
    def OnSpin(self, evt):
        """Update value based on spin button event."""
        new_val = self._min + evt.GetPosition() * self._inc
        new_val = max(min(new_val, self._max), self._min)
        self.SetValue(new_val)

    def OnMouseWheel(self, evt):
        """Adjust the value according to the mouse wheel movement."""
        rotation = evt.GetWheelRotation()
        new_val = self.Value + self._inc if rotation > 0 else self.Value - self._inc
        new_val = max(min(new_val, self._max), self._min)
        self.SetValue(new_val)
    ...
    def OnText(self, evt):
        """Handle text changes and validate them."""
        try:
            new_val = int(self.te.GetValue())
            new_val = max(min(new_val, self._max), self._min)
            self.SetValue(new_val)
        except ValueError:
            self.te.SetValue(str(self._committed))  # Reset to committed value on invalid input

    def SetValue(self, value):
        """Set the current display value to an integer."""
        if isinstance(value, int) and value != self._committed:
            self._committed = value
            self.te.ChangeValue(str(value))
            self.PostEvent()

    def GetValue(self):
        """Return the currently displayed value as an integer."""
        try:
            return int(self.te.GetValue())
        except ValueError:
    ...
    def CheckFocus(self, evt):
        """Check focus status to commit or cancel changes."""
        if not self.HasFocus() and not self.te.HasFocus():
            self.Cancel()
        else:
            self._checkTimer.StartOnce(500)
    ...
    def Commit(self):
        """Commit the pending value, if valid."""
        try:
            value = int(self.te.GetValue())
    ...
    def Cancel(self):
        """Cancel any pending changes and revert to the last committed value."""
        super().Cancel()
        self.te.SetValue(str(self._committed))
...
class SetPointGauge(SafeControl, wx.Window):
    ...
    def OnPaint(self, evt):
        ...
        if self._vertical:
            bar.width -= 2 * margin
            bar.X += margin
            bar.height = int(rect.height * self._value.to_quotient(self._displayed))
            bar.Y = rect.height - bar.height
        else:
            bar.height -= 2 * margin
            bar.Y += margin
            bar.width = int(rect.width * self._value.to_quotient(self._displayed))
        ...
        if self._pending is not None:
            ...
            if self._vertical:
                pos = int(rect.height * (1 - pos))
                dc.DrawLine(0, pos, rect.width, pos)
            else:
                pos = int(pos * rect.width)
